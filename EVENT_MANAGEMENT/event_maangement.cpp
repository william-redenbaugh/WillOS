...
static int **subscribed_eventspaces;
...
void event_management_thread(void *parameters){
    ...
    subscribed_eventspaces = (int**)malloc(sizeof(int*) * NUM_THREADS);
...
int subscribe_eventlist(int *event_list, int num_events, int event_length_max){
    ...
    subscribed_eventspaces[num_eventspaces] = (int*)malloc(sizeof(int) * num_events);
